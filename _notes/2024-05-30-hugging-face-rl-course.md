...
### The Big Picture and RL Learning Framework
...
Our discount rate determines the present value of future rewards[^9]: a reward received \\(q\\) steps into the future is worth \\(\gamma^q\\) times what it would be worth if received immediately. 

- At *zero steps* (meaning if it were received immediately, no discounting) we'd have:
...
- At *two steps* we'd have

	\\(R_{t} \cdot \gamma^{q}\\)

	\\(= R_{t} \cdot \gamma^{2}\\)

Now, just to throw some numbers into the mix to help get this idea into our heads, let's imagine that the reward is \\(10\\), and the discount factor is \\(0.9\\), we'd have the following:

- Immediate reward 

	\\(= R_{t} \cdot \gamma^{0} = 10\\)

- *One* step in the future

	\\(= R_{t} \cdot \gamma = 10 \cdot 0.9 = 9\\)

- *Two* steps in the future

	\\(= R_{t} \cdot \gamma^{2} = 10 \cdot 0.81 = 8.1\\)

**Quick note:** [Sutton and Barto](http://incompleteideas.net/book/RLbook2020.pdf) (on page 55), mention that: 

>"The discount rate determines the present value of the future rewards: a reward received \\(k\\) time steps in the future is worth \\(y^{k-1}\\)"
>
>*Sutton & Barto, 2020 (page 55)*

I didn't really get what they meant by this, as I reckoned it had to be \\(y^k\\) or \\(y^q\\), I assumed it had to be due to a difference in indexing. If time is expressed as \\(t + k\\) then we can see how this applies, for instance if we start at \\(t\\) and take one step forward in time we'd be at \\(t + 1\\), take another and you're at \\(t + 2\\) and so on. Therefore, if you're one step in the future (\\(t + 1\\)) then the *present* value of that reward is \\(y^{k-1} = y^{1-1} = y^{0} = 1\\) times the reward \\(R_t\\). However, as you can notice, it's common practice for our cumulative return to start at \\(R_{t+1}\\) and not \\(R_t\\). In short, you just need to know that this discounting allows us to set a present value for future rewards. 
...
[^9]: Due to indexing (the fact that we start our cumulative reward at t+1 and not t) the following section could get a bit confusing, so I'll start off with my version and then reference [Sutton and Barto](http://incompleteideas.net/book/RLbook2020.pdf) (listen to them lol, they're the experts) (page 55). 